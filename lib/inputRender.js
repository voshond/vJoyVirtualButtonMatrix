const inputRender = (inputs) => {
    ...
    inputs.map(input => {
        switch (input.type) {
            case "button":
                if (input.id === "") {
                    renderedHTML += `<button class="gridButton input ${input.class}" vjoytype="${input.type}" vjoyid="${i + 1}" style="${styleGenerator(input.style)}">${input.content}</button>`;
                } else {
                    renderedHTML += `<button class="gridButton input ${input.class}" vjoytype="${input.type}" vjoyid="${input.id}" style="${styleGenerator(input.style)}">${input.content}</button>`;
                }
                break;
            ...
            case "repeat":
                if (input.id === "") {
                    renderedHTML += `<button class="gridButton input ${input.class}" vjoytype="${input.type}" vjoyid="${i + 1}" vjoyinterval="${input.interval}" style="${styleGenerator(input.style)}">${input.content}</button>`;
                } else {
                    renderedHTML += `<button class="gridButton input ${input.class}" vjoytype="${input.type}" vjoyid="${input.id}" vjoyinterval="${input.interval}" style="${styleGenerator(input.style)}">${input.content}</button>`;
                }
                break;
            ...
            case "slider":
                if (input.id === "") {
                    ...
                    renderedHTML += `<div class="focus:outline-none grid relative ${input.class}">
